...
<head>
    ...
    <title>Détails de l'manga</title>
...
@include('partials.header')
    ...
    <div class="anime-header pt-12 relative w-full h-64 md:h-60 flex items-center justify-center mb-8">
        ...
        <div class="relative z-10 text-center px-4">
            ...
            <div class="flex justify-center items-center space-x-2 mt-4">
                ...
                <a href="{{ route('manga.chapitres.index', ['manga_id' => $manga->id]) }}"" class="bg-[#6B46C1] hover:bg-opacity-90 text-white font-bold py-2 px-6 rounded-button flex items-center transition-all duration-300 shadow-lg">
                    <i class="ri-play-circle-line mr-2"></i>
                    <span class="whitespace-nowrap">Voir les chapitres</span>
                </a>
            </div>
        </div>
    </div>
    ...
    <div class="container mx-auto px-4 max-w-6xl">
        <div class="flex items-center mb-8 text-sm">
            ...
            <span class="text-[#6B46C1] font-medium">{{ $manga->content->titre }}</span>
        </div>
        ...
        <div class="flex flex-col lg:flex-row gap-8">
            <div class="w-full lg:w-2/3">
                <div class="bg-black bg-opacity-70 rounded-lg shadow-md p-6 mb-8">
                    ...
                    <p class="text-violet-300 leading-relaxed">{{ $manga->content->description }}</p>
                </div>

                <div class="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
                    <div class="manga-card bg-black bg-opacity-70 p-4 rounded-lg shadow-md">
                        ...
                        <p class="font-medium text-violet-300">{{ $manga->content->type }}</p>
                    </div>
                    ...
                    <div class="manga-card bg-black bg-opacity-70 p-4 rounded-lg shadow-md">
                        ...
                        <p class="font-medium text-violet-300">{{ $manga->content->category->nom  }}</p>
                    </div>
                    ...
                    <div class="manga-card bg-black bg-opacity-70 p-4 rounded-lg shadow-md">
                        ...
                        <h3 class="text-xl text-white">Chapitres</h3>
                        <p class="font-medium text-violet-300">{{ $manga->nbr_chapitres }}</p>
                    </div>
                    ...
                    <div class="manga-card bg-black bg-opacity-70 p-4 rounded-lg shadow-md">
                        ...
                        <p class="font-medium text-violet-300">{{ $manga->date_debut }} - {{ $manga->date_fin }}</p>
                    </div>
                    ...
                    <div class="manga-card bg-black bg-opacity-70 p-4 rounded-lg shadow-md">
                        ...
                        <p class="font-medium text-violet-300">{{ $manga->auteur }}</p>
                    </div>
                </div>
                    ...
                    @if(Auth::check())
                        <div class="border-t pt-6">
                            ...
                            <form action="{{ route('commentaire.store') }}" method="POST" class="space-y-4">
                                @csrf
                                <input type="hidden" name="content_id" value="{{ $manga->content->id }}">
                                <div>
            ...
            <div class="w-full lg:w-1/3">
                <div class="bg-black bg-opacity-70 rounded-lg shadow-md overflow-hidden mb-8">
                    <img src="{{ asset($manga->content->photo) }}" class="w-full h-auto object-cover">
                </div>
                ...
                <div class="bg-black bg-opacity-70 rounded-lg shadow-md p-6 mb-8">
                    ...
                    @else
                        <div class="bg-gray-50 p-4 rounded-lg text-center">
                            <p class="text-white mb-2">Connectez-vous pour noter cet manga</p>
                            <a href="/login" class="inline-block bg-[#6B46C1] bg-opacity-10 text-[#6B46C1] font-medium py-2 px-4 rounded-button hover:bg-opacity-20 transition-colors !rounded-button whitespace-nowrap">Se connecter</a>
                        </div>
                    @endif
                </div>
                ...
                <div class="bg-black bg-opacity-70 rounded-lg shadow-md p-6">
                    ...
                    <ul class="space-y-3">
                        <li class="flex justify-between">
                            ...
                            <span class="font-medium text-violet-300">{{ $manga->content->datePublication }}</span>
                        </li>
                        <li class="flex justify-between border-t pt-3">
                            <span class="text-white">Type</span>
                            <span class="font-medium text-violet-300">{{ $manga->content->type }}</span>
                        </li>
                        <li class="flex justify-between border-t pt-3">
                            <span class="text-white">Genre</span>
                            <span class="font-medium text-violet-300">{{ $manga->content->category->nom }}</span>
                        </li>
                        <li class="flex justify-between border-t pt-3">
                            <span class="text-white">Chapitres</span>
                            <span class="font-medium text-violet-300">{{ $manga->nbr_chapitres }}</span>
                        </li>
