...
<!-- Form -->
<form action="{{ isset($editChapitre) ? route('manga.chapitres.update', $editChapitre->id) : route('manga.chapitres.store') }}" method="POST" class="space-y-4">
@csrf
...
<div>
<label for="nbr_pages" class="block text-sm font-medium text-gray-700 mb-1">Nombre de Pages</label>
<div class="relative">
...
<div>
<label for="date_ajout" class="block text-sm font-medium text-gray-700 mb-1">Date d'Ajout</label>
<div class="relative">
<div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
...
<button type="submit" class="bg-primary text-white px-4 py-2 !rounded-button hover:bg-opacity-90 transition-all whitespace-nowrap flex items-center">
...
<p class="text-gray-500 text-sm mt-2">Ajoutez votre premier chapitre en utilisant le formulaire ci-dessus.</p>
</div>
@else
...
<a href="{{ route('manga.chapitres.show', $chapitre->id) }}" class="inline-flex items-center px-3 py-1.5 bg-purple-50 text-purple-600 !rounded-button hover:bg-purple-100 whitespace-nowrap">
<i class="ri-eye-line mr-1"></i> Voir
</a>
<a href="{{ route('manga.chapitres.edit', $chapitre->id) }}" class="inline-flex items-center px-3 py-1.5 bg-fuchsia-50 text-fuchsia-600 !rounded-button hover:bg-fuchsia-100 whitespace-nowrap">
<i class="ri-edit-line mr-1"></i> Modifier
</a>
<form action="{{ route('manga.chapitres.destroy', $chapitre->id) }}" method="POST" class="inline-block">
@csrf
@method('DELETE')
<button type="submit" class="inline-flex items-center px-3 py-1.5 bg-red-50 text-red-600 !rounded-button hover:bg-red-100 whitespace-nowrap" onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce chapitre?')">
<i class="ri-delete-bin-line mr-1"></i> Supprimer
</button>
</form>
<a href="{{ route('manga.pages.create', ['chapitre_id' => $chapitre->id]) }}" class="inline-flex items-center px-3 py-1.5 bg-violet-50 text-violet-600 !rounded-button hover:bg-violet-100 whitespace-nowrap">
<i class="ri-add-line mr-1"></i> Ajouter pages
</a>
<a href="{{ route('manga.chapitres.pages.show', ['chapitre_id' => $chapitre->id]) }}" class="inline-flex items-center px-3 py-1.5 bg-purple-50 text-purple-600 !rounded-button hover:bg-purple-100 whitespace-nowrap">
...
<!-- Back Button -->
<div class="mt-8">
<a href="{{ route('content.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-100 text-gray-700 !rounded-button hover:bg-gray-200 transition-all whitespace-nowrap">
...
<button
type="button"
onclick="fermerCommentaireModal()"
