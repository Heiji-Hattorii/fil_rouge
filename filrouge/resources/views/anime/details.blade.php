...
<div class="container mx-auto p-4">
    ...
    <div class="bg-white rounded-lg shadow p-6 mb-6">
        ...
        <div class="flex space-x-1 mb-4">
            @for ($i = 1; $i <= 5; $i++)
                <span
                    class="star text-3xl {{ $i <= ($averageRating ?? 0) ? 'text-yellow-500' : 'text-gray-300' }}">&#9733;</span>
            @endfor
        </div>

        @if(Auth::check())
            <form action="{{ route('notation.store', ['contentId' => $anime->content->id]) }}" method="POST"
                class="space-y-4" onsubmit="return validateRating()">
                @csrf
                <label for="note" class="block text-lg">Donner une note (1 à 5) :</label>
                <div class="flex space-x-1">
                    @for ($i = 1; $i <= 5; $i++)
                        <span class="star-form text-3xl cursor-pointer text-gray-300 hover:text-yellow-500"
                            data-value="{{ $i }}" onclick="setRating(this, {{ $i }})">&#9733;</span>
                    @endfor
                </div>
        ...
        @if(Auth::check())
            <form action="{{ route('commentaire.store') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <textarea id="comment" name="comment" class="w-full p-2 border border-gray-300 rounded-md"
                        required></textarea>
                </div>
        ...
        <div class="flex space-x-4 mt-6">
            ...
            <a href="{{ route('anime.episodes.index', ['anime_id' => $anime->id]) }}"
