            ...
            @forelse($messages as $message)
                <li class="border-b p-2">
                    ...
                    @if($message->user_id == auth()->id())
                        <button type="button" onclick="toggleModal('editMessageModal{{ $message->id }}')" class="mt-2 p-2 bg-yellow-500 text-white rounded">Modifier</button>
                        <div id="editMessageModal{{ $message->id }}" class="modal hidden fixed inset-0 bg-gray-900 bg-opacity-50 flex justify-center items-center">
                            <div class="modal-content bg-white p-4 rounded shadow-lg w-1/2">
                                ...
                                <form action="{{ route('messages.update', $message) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <textarea name="message" class="w-full p-2 border rounded" required>{{ $message->message }}</textarea>
                                    <div class="mt-4 flex justify-end">
                                        <button type="button" class="btn-cancel p-2 bg-gray-500 text-white rounded" onclick="toggleModal('editMessageModal{{ $message->id }}')">Fermer</button>
                                        <button type="submit" class="btn-submit p-2 bg-blue-500 text-white rounded ml-2">Mettre à jour</button>
                                    </div>
                                </form>
                            </div>
                        </div>
                        <button type="button" onclick="toggleModal('deleteMessageModal{{ $message->id }}')" class="mt-2 p-2 bg-red-500 text-white rounded">Supprimer</button>
                        <div id="deleteMessageModal{{ $message->id }}" class="modal hidden fixed inset-0 bg-gray-900 bg-opacity-50 flex justify-center items-center">
                            <div class="modal-content bg-white p-4 rounded shadow-lg w-1/2">
                                <h5 class="text-xl">Supprimer le Message</h5>
                                <p>Êtes-vous sûr de vouloir supprimer ce message ?</p>
                                <div class="mt-4 flex justify-end">
                                    <button type="button" class="btn-cancel p-2 bg-gray-500 text-white rounded" onclick="toggleModal('deleteMessageModal{{ $message->id }}')">Annuler</button>
